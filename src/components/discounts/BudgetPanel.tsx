...
export const BudgetPanel: React.FC<BudgetPanelProps> = React.memo(({ 
  ...
  const mariliaPerc = getPercentualUtilizado(budget.marilia.totalUtilizado, budget.marilia.tetoMensal);
  ...
  return (
    <div className="corporate-card p-5 sm:p-6 mb-8 relative border-l-4 border-l-[#005b2e] bg-white shadow-xs">
      ...
      {/* Grid de Cards de Orçamento: Débora, Marília e Reserva Gerente */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        
        {/* Card Supervisora Débora Rodrigues */}
        <div className={`p-4 rounded-lg border transition-all ${
          budget.debora.saldoDisponivel <= 0
            ? 'bg-red-50/70 border-red-300'
            : budget.debora.saldoDisponivel < 80
            ? 'bg-amber-50/60 border-amber-300'
            : 'bg-white border-gray-200 hover:border-emerald-300'
        }`}>
          <div className="flex items-start justify-between mb-2">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-emerald-100 text-[#005b2e] font-bold flex items-center justify-center text-xs">
                DR
              </div>
              <div>
                <h3 className="text-sm font-bold text-gray-800">Débora Rodrigues</h3>
                <span className="text-[11px] text-gray-500 font-medium">Supervisora Inside Sales</span>
              </div>
            </div>
            {budget.debora.saldoDisponivel <= 0 ? (
              <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 bg-red-100 text-red-800 rounded-md border border-red-300">
                <AlertTriangle className="w-3 h-3" /> Teto Esgotado
              </span>
            ) : (
              <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 bg-emerald-50 text-[#005b2e] rounded-md border border-emerald-300">
                <CheckCircle className="w-3 h-3" /> Ativo ({ciclo.mesCurto})
              </span>
            )}
          </div>
          ...
          <div className="space-y-1.5 my-3">
            <div className="flex justify-between items-baseline text-xs">
              <span className="text-gray-500 font-medium">Saldo Mensal Restante:</span>
              <span className={`font-bold text-base ${budget.debora.saldoDisponivel <= 0 ? 'text-red-700' : 'text-[#005b2e]'}`}>
            ...
            <div className="flex justify-between text-[11px] text-gray-500 pt-0.5">
              <span>Utilizado: R$ {budget.debora.totalUtilizado.toFixed(2).replace('.', ',')}</span>
              <span>Teto: R$ {budget.debora.tetoMensal.toFixed(2).replace('.', ',')}/mês</span>
            </div>
          </div>

          {budget.debora.totalPendente > 0 && (
            <div className="mt-2 pt-2 border-t border-gray-100 flex items-center justify-between text-[11px] text-amber-700">
              <span className="flex items-center gap-1">
                <Clock className="w-3 h-3" /> Em análise:
              </span>
              <span className="font-semibold">R$ {budget.debora.totalPendente.toFixed(2).replace('.', ',')}</span>
            </div>
          )}
        </div>

        {/* Card Supervisora Marília Farias */}
        <div className={`p-4 rounded-lg border transition-all ${
          budget.marilia.saldoDisponivel <= 0
            ? 'bg-red-50/70 border-red-300'
            : budget.marilia.saldoDisponivel < 80
            ? 'bg-amber-50/60 border-amber-300'
            : 'bg-white border-gray-200 hover:border-emerald-300'
        }`}>
          <div className="flex items-start justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-sm font-bold text-gray-800">Marília Farias</h3>
                <span className="text-[11px] text-gray-500 font-medium">Supervisora Inside Sales</span>
              </div>
            </div>
            {budget.marilia.saldoDisponivel <= 0 ? (
          ...
          <div className="space-y-1.5 my-3">
            <div className="flex justify-between items-baseline text-xs">
              <span className="text-gray-500 font-medium">Saldo Mensal Restante:</span>
              <span className={`font-bold text-base ${budget.marilia.saldoDisponivel <= 0 ? 'text-red-700' : 'text-[#005b2e]'}`}>
                R$ {budget.marilia.saldoDisponivel.toFixed(2).replace('.', ',')}
              </span>
            </div>
            ...
            <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
              <div
                className={`h-2 rounded-full transition-all duration-500 ${
                  mariliaPerc >= 100 ? 'bg-red-600' : mariliaPerc > 75 ? 'bg-amber-500' : 'bg-primary-green'
                }`}
                style={{ width: `${mariliaPerc}%` }}
              ></div>
            </div>

            <div className="flex justify-between text-[11px] text-gray-500 pt-0.5">
              <span>Utilizado: R$ {budget.marilia.totalUtilizado.toFixed(2).replace('.', ',')}</span>
              <span>Teto: R$ {budget.marilia.tetoMensal.toFixed(2).replace('.', ',')}/mês</span>
            </div>
          </div>

          {budget.marilia.totalPendente > 0 && (
            <div className="mt-2 pt-2 border-t border-gray-100 flex items-center justify-between text-[11px] text-amber-700">
              <span className="flex items-center gap-1">
                <Clock className="w-3 h-3" /> Em análise:
              </span>
              <span className="font-semibold">R$ {budget.marilia.totalPendente.toFixed(2).replace('.', ',')}</span>
            </div>
          )}
        </div>
        ...
        <div className={`p-4 rounded-lg border relative overflow-hidden transition-all ${
          ...
          <div className="flex items-start justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-sm font-bold text-gray-800">Heder Santos</h3>
                <span className="text-[11px] text-emerald-800 font-medium">Gerente de Vendas (Aprovador)</span>
              </div>
            </div>
            {budget.reservaGerente <= 0 ? (
              <span className="inline-flex items-center text-[10px] font-bold px-2 py-0.5 bg-red-100 text-red-800 rounded-md border border-red-300">
                Teto Esgotado ({ciclo.mesCurto})
              </span>
            ) : (
              <span className="inline-flex items-center text-[10px] font-bold px-2 py-0.5 bg-emerald-50 text-emerald-800 rounded-md border border-emerald-300">
          ...
          <div className="space-y-1.5 my-3">
            <div className="flex justify-between items-baseline text-xs">
              <span className="text-gray-600 font-medium">Saldo Restante ({ciclo.mesCurto}):</span>
              <span className={`font-bold text-base ${budget.reservaGerente <= 0 ? 'text-red-700' : 'text-[#005b2e]'}`}>
            ...
            <div className="flex justify-between text-[11px] text-gray-500 pt-0.5">
              <span>Utilizado: R$ {(budget.gerente?.totalUtilizado || 0).toFixed(2).replace('.', ',')}</span>
              <span>Teto: R$ {(budget.gerente?.tetoMensal || 100).toFixed(2).replace('.', ',')}/mês</span>
            </div>

            {budget.reservaGerente <= 0 ? (
              <div className="p-2 bg-red-100/70 border border-red-300 rounded text-[11px] text-red-900 mt-2 flex items-start gap-1.5 font-medium">
                <AlertOctagon className="w-3.5 h-3.5 text-red-600 shrink-0 mt-0.5" />
                <span>O teto mensal do Gerente (R$ 100,00) em {ciclo.mesCurto} foi atingido. Próxima liberação em {ciclo.dataProximaRenovacao}.</span>
              </div>
            ) : (
